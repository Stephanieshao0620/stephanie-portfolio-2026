...
function comparisonBar(label, value) {
  return `<div style="--value:${value}%"><dt>${label}</dt><i aria-hidden="true"></i><dd>${value}%</dd></div>`;
}
...
function donorPage() {
  const t = zh ? {
    ...
    lensesLabel: 'TWO DONOR LENSES', lensesTitle: '同樣支持，期待的內容不同', lensesBody: '這兩種畫像用來理解內容偏好，不等於固定身分標籤。', inside: 'INSIDER LENS', insideQuote: '「我是多多的一部分」', insideBody: '在意專業手藝，也在意團隊如何工作。', outside: 'VALUE LENS', outsideQuote: '「我讓多多繼續存在」', outsideBody: '與議題牽著線，更容易先被人物故事吸引。',
    ...
    lensesLabel: 'TWO DONOR LENSES', lensesTitle: 'Same support, different content expectations', lensesBody: 'These portraits help organize content preferences. They are not fixed identity labels.', inside: 'INSIDER LENS', insideQuote: '“I am part of Right Plus.”', insideBody: 'They care about professional craft and how the team works.', outside: 'VALUE LENS', outsideQuote: '“I help Right Plus continue.”', outsideBody: 'Connected by the issue, they are more readily drawn in by personal stories.',
  ...
  <section class="report-section sand">${sectionHeading(t.lensesLabel, t.lensesTitle, t.lensesBody)}<div class="comparison-grid"><article class="comparison light" data-reveal><span>${t.inside}</span><h3>${t.insideQuote}</h3><p>${t.insideBody}</p><dl>${comparisonBar(zh?'分析與方法論':'Analysis and methods',38)}${comparisonBar(zh?'人物故事':'Personal stories',29)}${comparisonBar(zh?'幕後日常':'Behind the scenes',23)}${comparisonBar(zh?'關鍵數據':'Key metrics',10)}</dl></article><article class="comparison dark" data-reveal><span>${t.outside}</span><h3>${t.outsideQuote}</h3><p>${t.outsideBody}</p><dl>${comparisonBar(zh?'人物故事':'Personal stories',52)}${comparisonBar(zh?'分析與方法論':'Analysis and methods',28)}${comparisonBar(zh?'關鍵數據':'Key metrics',10)}${comparisonBar(zh?'幕後日常':'Behind the scenes',10)}</dl></article></div></section>
